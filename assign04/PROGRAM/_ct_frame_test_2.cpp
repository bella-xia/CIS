...
/**
 * helper function to generate random rotational tranformation
 */
...
/**
 * helper function to generate random translational tranformation
 */
...
{
    srand(time(0));
    // intialize all the constants
    const Matrix pos = Matrix(10, 17, 28);
    const double radius = 10;
    const double noise = 0.001;

    // initialize vectors to store the local frame points (Mat_zero) and the global frame
    // points for different frame transformations based on pivot rotation
    ...
    // initialize random rotational transfromation, use it to simulate the rotational
    // movements of the tip. Then calculate each point's coordinates on the global frame
    // (with respect to a point with translational transformation pos from current tip)
    for (int i = 0; i < 10; ++i)
    {
    ...
    // iteratively push back the global frame points for each frame, estimate a frame transformation
    ...
    // collect all the frame transformations to perform pivot calibration. Then separate the result
    // into the pointer with respect to local frame and with respect to the global point of reference
    ...
    // generate a random frame transformation
    ...
    // for each instance, generate a frame transformation, construct the points before transformation
    // with random noises, then use point-cloud registration to estimate the frame
    ...
    // use the estimated frame transformation on a new set of points
